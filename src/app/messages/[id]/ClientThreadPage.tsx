...
export default function ClientThreadPage({ id }: { id: string }) {
  ...
  const { data: thread } = useQuery({ queryKey: ['message-thread', id], queryFn: async () => (await apiFetch(`/api/message-threads/${id}`)).json(), enabled: !!id });
  const { data: messagesData } = useQuery({ queryKey: ['messages', id], queryFn: async () => (await apiFetch(`/api/messages?threadId=${id}`)).json(), enabled: !!id, refetchInterval: isGhPages ? false : 3000 });
  const messages = messagesData?.data || [];

  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [messagesData]);

  const uploadAttachmentMutation = useMutation({
    mutationFn: async (file: File) => (await apiFetch('/api/attachments/upload', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: file.name, type: file.type.startsWith('image/') ? 'image' : file.type === 'application/pdf' ? 'pdf' : file.type.startsWith('video/') ? 'video' : 'document', size: file.size, mimeType: file.type }) })).json()
  });

  const sendMessageMutation = useMutation({
    mutationFn: async (messageData: any) => (await apiFetch('/api/messages', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(messageData) })).json(),
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Button variant="outlined" startIcon={<DownloadIcon />} onClick={() => apiFetch('/api/messages/export', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ threadId: id }) })}>Export to PDF</Button>
          </Box>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-end' }}>
            ...
            <Button variant="contained" onClick={handleSendMessage} disabled={isGhPages || (!newMessage.trim() && selectedFiles.length === 0)}><SendIcon /></Button>
          </Box>
        </CardContent>
      </Card>
    </Box>
